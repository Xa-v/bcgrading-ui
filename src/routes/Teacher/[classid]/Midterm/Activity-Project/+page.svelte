  ...
  onMount(async () => {
     ...
     await import('bootstrap/dist/js/bootstrap.bundle.min.js');
      ...
      const token = localStorage.getItem('jwtToken');
          ...
          const prelimactivityproject = await fetch(`http://localhost:4000/teacher/Midterm/Activity/${classid}`, {
              headers: {
                  'Authorization': `Bearer ${token}` // Include JWT token
              }
          });
          ...
          if (prelimactivityproject.ok) {
            activityprojectmidterm = await prelimactivityproject.json();
          } else {
...
<!-- <div class="card rounded-0 shadow-sm mb-2">
  <div class="card-body "> -->
      ...
      <ul class="nav nav-tabs rounded-0 d-flex" id="myTab" role="tablist">
          ...
          <li class="nav-item flex-grow-1">
              <a class="nav-link active disabled rounded-0 border text-center" href={`/Teacher/${classid}/Midterm/Activity-Project`} role="tab" aria-selected="true">ACTIVITY/PROJECT</a>
          </li>
          <li class="nav-item flex-grow-1">
              <a class="nav-link rounded-0 border text-center" href={`/Teacher/${classid}/Midterm/Exam`} role="tab" aria-selected="true">EXAM</a>
          </li>
      </ul>
      ...
      <div class="row">
        ...
        <div class="col-md-8" style="max-height: 50vh;">
            <div class="row mt-3">
                {#each activityprojectmidterm as activityproject}
                <div class="col-md-3">
                    <div class="acard card rounded-0 text-center mb-3 shadow-sm">
                        <a href={`/Teacher/${classid}/Midterm/Activity-Project/${activityproject.gradeid}`} style="text-decoration: none; color: inherit;">
                          <div class="card-body p-2 pb-4">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    <p class="m-0 p-2">{activityproject.scoretype}</p>
                                </div>
                                <div class="dropdown">
                            ...
                            <strong class="mt-2 pt-2"> {new Date(activityproject.created).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</strong>
                        </div>

<script>
    ...
    let classinfo = [];
    let activityprojectscoresmidterm = [];
    let error = '';
    let userRole = '';
       ...
       await import('bootstrap/dist/js/bootstrap.bundle.min.js');
        ...
        const token = localStorage.getItem('jwtToken');
            ...
            if (activityprojectscores.ok) {
                activityprojectscoresmidterm = await activityprojectscores.json();
            } else {
    ...
    <div class="card rounded-0 shadow-sm mb-2">
      <div class="card-body">
        ...
        <div class="d-flex justify-content-start gap-4 mb-1">
          ...
          <a class="btn btn-sm rounded-0 btn-primary  p-2 mb-3"  href={`/Teacher/${1}/Midterm/Activity-Project`}>
        ...
        <ul class="nav nav-tabs rounded-0 d-flex" id="myTab" role="tablist">
          ...
          <li class="nav-item flex-grow-1">
            <a class="nav-link active disabled rounded-0 border  text-center" href={`/Teacher/${classid}/Midterm/Activity-Project`} role="tab" aria-selected="false">ACTIVITY/PROJECT</a>
        </li>
        <li class="nav-item flex-grow-1">
         ...
         <div style="max-height: 42vh; overflow-y: auto; overflow-x: hidden;">
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                    {#each activityprojectscoresmidterm as activityproject}
                    <tr>
                        <td>{activityproject.scoreid}</td>
                        <td>{activityproject.gradeid}</td>
                        <td>{activityproject.term}</td>
                        <td>{activityproject.scoretype}</td>
                        <td>{activityproject.score}</td>
                        <td>{activityproject.perfectscore}</td>
                        <td>{activityproject.Studentlist?.studentinfo?.lastName}</td>
                        <td>{activityproject.Studentlist?.studentinfo?.firstName}</td>
                        <td>{activityproject.Studentlist?.studentinfo?.id}</td>
                    </tr>
                    {/each}
                </tbody>
            </table>

  ...
  onMount(async () => {
     ...
     await import('bootstrap/dist/js/bootstrap.bundle.min.js');
      ...
      const token = localStorage.getItem('jwtToken');
          ...
          const midtermsattendance = await fetch(`http://localhost:4000/teacher/Final/Attendance/${classid}`, {
              headers: {
                  'Authorization': `Bearer ${token}` // Include JWT token
              }
          });
          ...
          if (midtermsattendance.ok) {
            attendancefinal = await midtermsattendance.json();
          } else {
...
<div class="card rounded-0 shadow-sm mb-2">
  <div class="card-body">
      ...
      <ul class="nav nav-tabs rounded-0 d-flex" id="myTab" role="tablist">
          ...
          <li class="nav-item flex-grow-1">
              <a class="nav-link rounded-0 text-center active" href={`/Teacher/${classid}/Final`} role="tab" aria-selected="false">FINAL</a>
          </li>
      </ul>

      <ul class="nav nav-tabs rounded-0 d-flex" id="myTab" role="tablist">
          <li class="nav-item flex-grow-1">
              <a class="nav-link active disabled rounded-0 border text-center" href={`/Teacher/${classid}/Final/Attendance`} role="tab" aria-selected="false">ATTENDANCE</a>
          </li>
          <li class="nav-item flex-grow-1">
      ...
      <div class="row">
          ...
          <div class="col-md-8" style="max-height: 50vh; overflow-y: auto;">
              <div class="row mt-3">
                  {#each attendancefinal as attendance}
                  <div class="col-md-4">

    ...
    onMount(async () => {
       ...
       await import('bootstrap/dist/js/bootstrap.bundle.min.js');
        ...
        const token = localStorage.getItem('jwtToken');
            ...
            // }
            const participationscores = await fetch(`http://localhost:4000/teacher/getgradelist/${gradeid}`, {
                headers: {
                    'Authorization': `Bearer ${token}` // Include JWT token
                }
            });

            if (participationscores.ok) {
                participationscoresprelim = await participationscores.json();
            } else {
                error = `Failed to fetch participationscores: ${participationscores.statusText}`;
            }

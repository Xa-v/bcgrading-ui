<script>
  ...
  let classinfo = [];
  let participationprelim = [];
  let error = '';
  let successMessage = '';
  let userRole = '';

  let term = 'Prelim';  // Default to Prelim
  let scoretype = 'Participation';  // Default to Attendance
    ...
    sessionStorage.clear();
     ...
     await import('bootstrap/dist/js/bootstrap.bundle.min.js');
      ...
      const token = localStorage.getItem('jwtToken');
          ...
          const prelimsparticipation = await fetch(`http://localhost:4000/teacher/Prelim/Participation/${classid}`, {
              headers: {
                  'Authorization': `Bearer ${token}` // Include JWT token
              }
          });
          ...
          if (prelimsparticipation.ok) {
              participationprelim = await prelimsparticipation.json();
          } else {
  ...
  async function handleSubmit(event) {
      ...
      try {
          ...
          if (response.ok) {
              successMessage = 'Participation Score Successfully added';
              error = '';
  ...
  <div class="card-body">
      ...
      <ul class="nav nav-tabs rounded-0 d-flex" id="myTab" role="tablist">
        <li class="nav-item flex-grow-1">
            <a class="nav-link rounded-0 text-center" href={`/Teacher/${classid}`} role="tab" aria-selected="false">GRADES</a>
          </li>
          <li class="nav-item flex-grow-1">
      ...
      <ul class="nav nav-tabs rounded-0 d-flex" id="myTab" role="tablist">
          ...
          <li class="nav-item flex-grow-1">
              <a class="nav-link active disabled  rounded-0 border text-center" href={`/Teacher/${classid}/Prelim/Participation`} role="tab" aria-selected="false">PARTICIPATION</a>
          </li>
          <li class="nav-item flex-grow-1">
      ...
      <div class="row">
          ...
          <div class="col-md-8" style="max-height: 50vh; overflow-y: auto;">
              <div class="row mt-3">
                  {#each participationprelim as participation}
                  <div class="col-md-4">
                      <div class="acard card rounded-0 text-center mb-3 shadow-sm" style="width: 15rem; height: 7rem;">
                          <a href={`/Teacher/${classid}/Prelim/Participation/${participation.gradeid}`} style="text-decoration: none; color: inherit;">
                            <div class="card-body p-2">
                              <strong class="d-block">{participation.scoretype}</strong>
                              <strong class="d-block">Perfect Score: {participation.perfectscore}</strong>
                              <strong class="d-block border-top border-dark"> {new Date(participation.created).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</strong>
                          </div>
          ...
          <div class="col-md-4">
              <form class="form-inline mt-3 ms-auto" on:submit|preventDefault={handleSubmit} style="width: 90%;">
                  ...
                  <!-- Perfect Score Input -->
                   <div class="card-header">Add Participation score/grade</div>
                  ...
                  <!-- Submit Button -->
                  <div class="ms-3 mt-2">
                      <button class="btn btn-primary btn-sm" type="submit">Add Score</button>
                  </div>

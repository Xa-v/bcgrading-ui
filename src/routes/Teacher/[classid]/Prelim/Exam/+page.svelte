<script>
  ...
  let scoretype = 'Exam';  // Default to Exam
    ...
    sessionStorage.clear();
     ...
     await import('bootstrap/dist/js/bootstrap.bundle.min.js');
      ...
      const token = localStorage.getItem('jwtToken');
          ...
          const prelimexam = await fetch(`http://localhost:4000/teacher/Prelim/Exam/${classid}`, {
              headers: {
                  'Authorization': `Bearer ${token}` // Include JWT token
              }
          });
          ...
          if (prelimexam.ok) {
              examprelim = await prelimexam.json();
          } else {
              error = `Failed to fetch prelimexam: ${prelimexam.statusText}`;
          }
  ...
  async function handleSubmit(event) {
      ...
      try {
          ...
          if (response.ok) {
              successMessage = 'Exam Score Successfully added';
              error = '';
...
<!-- <div class="card rounded-0 shadow-sm mb-2">
  <div class="card-body"> -->
      ...
      <ul class="nav nav-tabs rounded-0 d-flex" id="myTab" role="tablist">
          ...
          <li class="nav-item flex-grow-1">
              <a class="nav-link active disabled rounded-0 border text-center" href={`/Teacher/${classid}/Prelim/Exam`} role="tab" aria-selected="false">EXAM</a>
          </li>
      </ul>
      ...
      <div class="row">
        ...
        <div class="col-md-8" style="max-height: 50vh;">
            <div class="row mt-3">
                {#each examprelim as examprelim}
                <div class="col-md-3">
                    <div class="acard card rounded-0 text-center mb-3 shadow-sm">
                        <a href={`/Teacher/${classid}/Prelim/Exam/${examprelim.gradeid}`} style="text-decoration: none; color: inherit;">
                          <div class="card-body p-2 pb-4">
                            <div class="d-flex justify-content-between align-items-center">
                                <div>
                                    <p class="m-0 p-2">{examprelim.scoretype}</p>
                                </div>
                                <div class="dropdown">
                            ...
                            <strong class="mt-2 pt-2"> {new Date(examprelim.created).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</strong>
                        </div>
          ...
          <div class="col-md-4">
              <form class="form-inline mt-3 ms-auto" on:submit|preventDefault={handleSubmit} style="width: 90%;">
                  ...
                  <!-- Perfect Score Input -->
                   <div class="card-header">Add Exam score/grade</div>

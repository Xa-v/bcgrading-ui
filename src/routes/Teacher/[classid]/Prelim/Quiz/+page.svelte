  ...
  onMount(async () => {
     ...
     await import('bootstrap/dist/js/bootstrap.bundle.min.js');
      ...
      const token = localStorage.getItem('jwtToken');
          ...
          const prelimsquiz = await fetch(`http://localhost:4000/teacher/Prelim/Quiz/${classid}`, {
              headers: {
                  'Authorization': `Bearer ${token}` // Include JWT token
              }
          });
          ...
          if (prelimsquiz.ok) {
              quizprelim = await prelimsquiz.json();
          } else {
              error = `Failed to fetch prelimsquiz: ${prelimsquiz.statusText}`;
          }
...
<div class="card rounded-0 shadow-sm mb-2">
  <div class="card-body">
      ...
      <ul class="nav nav-tabs rounded-0 d-flex" id="myTab" role="tablist">
          ...
          <li class="nav-item flex-grow-1">
              <a class="nav-link active disabled rounded-0 border text-center" href={`/Teacher/${classid}/Prelim/Quiz`} role="tab" aria-selected="false">QUIZ</a>
          </li>
          <li class="nav-item flex-grow-1">
      ...
      <div class="row">
          ...
          <div class="col-md-8" style="max-height: 50vh; overflow-y: auto;">
              <div class="row mt-3">
                  {#each quizprelim as quiz}
                  <div class="col-md-4">
                      <div class="acard card rounded-0 text-center mb-3 shadow-sm" style="width: 15rem; height: 7rem;">
                          <a href={`/Teacher/${classid}/Prelim/Quiz/${quiz.gradeid}`} style="text-decoration: none; color: inherit;">
                            <div class="card-body p-2">
                              <strong class="d-block">{quiz.scoretype}</strong>
                              <strong class="d-block">Perfect Score: {quiz.perfectscore}</strong>
                              <strong class="d-block border-top border-dark"> {new Date(quiz.created).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</strong>
                          </div>
